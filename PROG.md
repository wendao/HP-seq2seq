...
**最大化 `score = val_hit_rate - 0.5 * train_hit_rate`**

这是对纯 val_hit_rate 的修正：用 train_hit_rate 作为过拟合惩罚项。当 train 和 val 之间的差距很大时（过拟合），score 会被拉低，迫使搜索偏向泛化更好的配置。

> 之前仅优化 val_hit_rate 导致选中了过拟合的配置（train 虚高、val 尚可），实际泛化能力差。引入该惩罚项后，优先选择 train/val 差距小且 val 不低的模型。
...
Best score (val - 0.5*train): 0.2301
```

从日志中提取关键指标：

```bash
grep "Best score" run.log
```
...
3. train_hit_rate
4. score = val_hit_rate - 0.5 * train_hit_rate——**这是优化目标，status 基于此判断**
5. 峰值显存（GB），保留 1 位小数——崩溃用 0.0
6. 状态：`keep`（score 优于基线）、`discard`（score 差于或等于基线）、`crash`
7. 简短描述
...
```
commit	val_hit_rate	train_hit_rate	score	memory_gb	status	description
30798ac	0.4032	0.5100	0.1482	0.0	keep	baseline: HIDDEN=512 EMBED=96 LAYERS=4
c4fc6ef	0.4525	0.4200	0.2425	0.0	keep	warmup+cosine LR scheduler
735a00e	0.2971	0.3500	0.1221	0.0	discard	DROPOUT=0.0
6cb65d8	0.0000	0.0000	0.0000	0.0	crash	NUM_HEADS=8 OOM
```
...
8. 如果 **score** 提高，保留 commit，推进分支
9. 如果 score 持平或更差，`git reset --hard HEAD~1` 回退
...
### 1.3 有效修改的判定标准（基于 score = val - 0.5*train）
...
### 5.1 对 train/val 差距的误解

- Train hit rate 通常远高于 val hit rate（差距可达 0.1-0.15）
- 优化目标是 **score = val_hit_rate - 0.5 * train_hit_rate**，显式惩罚过拟合
- 不要单独以 val hit rate 或 train hit rate 作为判断标准，应看 score
- 一个 train=0.90, val=0.55 的模型（score=0.10）不如 train=0.50, val=0.48（score=0.23）
...
满足以下条件时可以认为已进入收益递减区：
- 连续 5-8 个实验的 score 都没有超过当前最佳
- 已覆盖所有主要超参数维度
- 最佳 score 在多次运行中稳定（考虑随机波动）

此时应：
- 记录最终最佳配置（含 score、val_hit_rate、train_hit_rate）
- 更新 AUTO.md 和 PROG.md
- 准备汇报
...
**当前最佳 val_hit_rate：约 0.45-0.47**（存在随机波动；代码已加 seed=42）
**优化目标：score = val_hit_rate - 0.5 * train_hit_rate**（防止过拟合）
